...
parser = argparse.ArgumentParser(description='CBAM')
...
parser.add_argument('--dataPath', type=str, default='./save/centroids', help='Path for centroid data.')
parser.add_argument('--runsPath', type=str, default='./save/runs', help='Path to save runs to.')
parser.add_argument('--savePath', type=str, default='checkpoints',
        help='Path to save checkpoints to in logdir. Default=checkpoints/')
parser.add_argument('--cachePath', type=str, default="./save/", help='Path to save cache to.')
...
parser.add_argument('--arch', type=str, default='resnet',
        help='basenetwork to use', choices=['vgg16', 'alexnet', 'resnet', 'resnet_cbam', 'vgg16_cbam'])
...
if __name__ == "__main__":
    ...
    elif opt.dataset.lower() == 'wurenji':
        import wurenji as dataset
    else:
    ...
    if opt.mode.lower() == 'train':
        ...
        elif opt.split.lower() == 'wurenji':
            whole_test_set = dataset.my_own_training_set()
            print('===> Evaluating on new set')
    ...
    if opt.arch.lower() == 'alexnet':
    ...
    elif opt.arch.lower() == 'resnet_cbam':
        encoder_dim = 512
        encoder = models.resnet34(pretrained=pretrained)
        # capture only feature part and remove last relu and maxpool
        layers = list(encoder.children())[:-2]
        if pretrained:
            ...
            for l in layers[:-2]:
                for p in l.parameters():
                    p.requires_grad = False
        layers.append(CBAM(512))

    elif opt.arch.lower() == 'vgg16_cbam':
